import {execFileSync} from 'node:child_process';import {fileURLToPath} from 'node:url';import {fixtures} from '../lib/fixtures';import {fallbackExtract,claimsFromExtraction} from '../lib/extract';import {verifyClaims} from '../lib/resolver';import type {Claim,Verdict} from '../lib/types';
const cases=[
...
const expected=(name:string)=>({
 court_name:name==='degraded OCR text proxy'?'':'SUPERIOR COURT OF CALIFORNIA, COUNTY OF RIVERSIDE',
 juror_or_reference_number:name==='nonexistent private identifier'?'99999999':name==='degraded OCR text proxy'?'':'10472893',
 phone_numbers:name==='degraded OCR text proxy'||name==='single field altered phone'?'8665550199':name==='unsupported jurisdiction'?'8665550199':name==='official-source conflict'?'7603426264':'9512755076',
 case_or_docket_number:'',
 delivery_method:'',
 court_location:name==='degraded OCR text proxy'?'':name==='unsupported jurisdiction'?'12 Cedar Street, Northbridge':'Riverside Historic Courthouse, 4050 Main Street, Riverside, CA 92501',
 urls:name==='single field altered portal'?'rcvduty.com':name==='unsupported jurisdiction'||name==='degraded OCR text proxy'?'':'jurywest.riverside.courts.ca.gov',
 emails:'',
 payment_amount:name==='generic paper fee'?'$50':''
});
for(const item of cases){const e=fallbackExtract(item.text),c=claimsFromExtraction(e,item.text),v=await verifyClaims(c,e.court_name,'SNAPSHOT');const reds=v.results.filter(r=>r.verdict==='MISMATCH').length;const exact=v.results.every(r=>r.verdict===expectedVerdict(item.name,c.find(x=>x.id===r.claim_id)!));total+=v.results.length;unknown+=v.results.filter(r=>r.verdict==='COULD_NOT_VERIFY').length;mismatch+=reds;trueMismatch+=Math.min(reds,item.expectedMismatch);falseMismatch+=Math.max(0,reds-item.expectedMismatch);const ground=expected(item.name);
