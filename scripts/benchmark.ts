...
const cases=[
 {name:'action-first jury message',text:fixtures['action-message-demo'].text,expectedMismatch:3},
...
function expectedVerdict(name:string,c:Claim):Verdict{
 if(name==='action-first jury message')return c.type==='court'?'MATCH':['phone','payment','information'].includes(c.type)?'MISMATCH':'COULD_NOT_VERIFY';
