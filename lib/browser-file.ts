...
export type BrowserDocument={text:string;tokens:Token[];preview:string;kind:'image'|'pdf';uncertain:boolean};
export async function readInBrowser(file:File):Promise<BrowserDocument>{
 ...
 if(file.type==='application/pdf'){
  ...
  const doc=await pdfjs.getDocument({data:await file.arrayBuffer()}).promise;const pages:string[]=[];const tokens:Token[]=[];
  ...
  const recognized=await ocr(canvas);return {...recognized,preview,kind:'pdf'};
 }
 const image=new Image();image.src=preview;await image.decode();return {...await ocr(image),preview,kind:'image'};
}
async function ocr(image:HTMLImageElement|HTMLCanvasElement):Promise<Omit<BrowserDocument,'preview'|'kind'>>{
