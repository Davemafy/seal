...
export default function SealApp({initialDemo=false}:{initialDemo?:boolean}){
 ...
 async function run(sourceMode:Mode=mode){
  ...
  try{
   ...
   }else{setRevealed(found.length);const requested=found.find(c=>c.action&&checked.results.find(r=>r.claim_id===c.id)?.verdict==='MISMATCH')||found.find(c=>c.action)||found.find(c=>checked.results.find(r=>r.claim_id===c.id)?.verdict==='MISMATCH')||found[0];setSelected(requested.id)}
