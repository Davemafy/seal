...
Nine deterministic cases cover consistent, altered, missing, unsupported, degraded-text proxy, prompt injection, generic paper payment, and conflicting official number. `npm run benchmark` reports zero false MISMATCH, 100% precision and 100% expected-state success on this small fixture set. The extraction accuracy figures are fixture-level checks, not a real-world OCR estimate. A genuine unmodified jury summons and degraded real-world photo corpus are still needed for external validity.
