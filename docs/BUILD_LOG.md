...
5. **Deploy/harden — in progress.** Inspected the deployed desktop before and after the nine-claim reveal. Exercised production live-source failure: nine conservative unverified verdicts, an explicit failure message, and the dated snapshot court contact remained visible. GitHub Actions installed Chromium and passed typecheck, lint, unit tests, and benchmark, but the browser test did not see a result after an early click. Reproduced that on production immediately after navigation: the first click arrived before React hydration; a later click worked. Disabled the primary buttons until hydration attaches, switched CI’s browser test to a production build, and added a 390px claim-to-evidence check. The next CI run is the remaining gate. Local Chromium remains unavailable in this workspace.
